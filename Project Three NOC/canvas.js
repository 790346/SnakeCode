...
function loadSegments(segments){
  for(var i = 0; i<segments; i++){
    ...
    var dx = 3;
    var dy = 0;
    var loc = new JSVector(orb.loc.x, orb.loc.y);
    var vel = new JSVector(orb.vel.x, orb.vel.y);
    ctx.strokeStyle = randomColor();
    ctx.fillStyle = randomColor();
    snake.push(new Snake(loc, vel, randomColor(), segments[i]));
  }
}
...
function loadMovers(numMovers){
  for(var i=0; i<numMovers; i++){
        ...
        snake = new Snake(new JSVector(10,10), new JSVector(3,3), randomColor(), segments);
}
